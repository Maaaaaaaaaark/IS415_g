...
```{R}
pacman::p_load(tidyverse, sf, tmap, maptools, spatstat, spNetwork, classInt, viridis, arrow,dplyr,lubridate,sparr,raster,stpp,osmdata)
```
...
```
Using appropriate tmap functions
```{r}
kde_acledMy_ppp_list <- list()
...
for (i in 1:4) {
  acled_quarter_ppp <- acledMy_ppp[acledMy_ppp$marks == i]  
  kde_acledMy_ppp_list[[i]] <- density(acled_quarter_ppp, kernel="gaussian")
  kde_raster_list[[i]] <- raster(kde_acledMy_ppp_list[[i]])
  crs(kde_raster_list[[i]]) <- CRS("+init=EPSG:32647")
}
tmap_mode("view")
basemap <- tm_shape(boundry) +
  tm_borders() +
  tm_tiles("OpenStreetMap")
for (i in 1:4) {
  kde_layer <- tm_shape(kde_raster_list[[i]]) +
    tm_raster(style = "cont", palette = "-RdYlBu", alpha = 0.7, title = paste("KDE Quarter", i))
  print(basemap + kde_layer)
}
