...
It is the distance between the nodes. This function compute this distance using a data.frame with observations vector in each node.
```{r}
lcosts <- nbcosts(shan.nb, shan_ict)
```
For each observation, this gives the pairwise dissimilarity between its valuse on the five variables and the values for the neighbouring observation. 
Basically, this is the notion if a generalised weight for a spatial weights matrix.
...
dim(shan.mst)
```
the dimension is 54 and not 55. This is because the minimum spanning tree cosistson n-1 edges(links) in order to traverse all the nodes
