...
ClusterGeo is an R package specially designed to support the need of performing spatially constrained cluster analysis.
And it also provides a Ward-like hierarchical clustering algorithm called hclustgeo() including spatial/geographical constrains
The idea is then to determine a value of alpha which increase the spatial contiguity without deteriorting too much the quality of the solution based on the variables of interest. This need is supported by a function called choicealpha()
### 6.2 Ward-like hierarchical clustering: ClustGeo
ClustGeo package provides function called hclusegeo() to perform a typical Ward-likeb hierarchical clustering just like hclust().
...
rect.hclust(nongeo_cluster,
            ...
            border = 2:5)
```
the dissimilarity matrix must be an object of class dist.
#### 6.2.1 Mapping the clister formed
```{r}
